...
class HrtFrame {
  ...
  String get frame {
    ...
    try {
      ...
      resp = preamble +
          ...
          address +
          command +
          nBBody.toRadixString(16).padLeft(2, '0') +
          _body +
          calcCheckSum();
    } catch (e) {
    ...
    try {
      ...
      if (_isLongFrame == false) {
        nBBody = int.parse(strFrame.substring(posIniFrame + 6, posIniFrame + 8),
            radix: 16);
        _body =
            strFrame.substring(posIniFrame + 8, posIniFrame + 8 + 2 * nBBody);
      } else {
        nBBody = int.parse(
  ...
  set isLongFrame(bool newIsLongFrame) {
    ...
    final value = int.parse(delimiter, radix: 16);
    value.setBits(7, 1, _isLongFrame == true ? 1 : 0);
    delimiter = value.toRadixString(16).padLeft(2,'0');
  }
}
